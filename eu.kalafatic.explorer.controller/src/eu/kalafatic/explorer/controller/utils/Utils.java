...
public class Utils {
	...
	public static Device getGateway() {
		final Device gateway = ExplorerFactory.eINSTANCE.createDevice();
		gateway.setIp("Gateway");
		gateway.setHost("Gateway");

		Set<Object> set = NetUtils.getGateway();
		int row = 0;
		for (Object object : set) {
			try {
				List<String> parsedData = (List<String>) object;
				final Route route = ExplorerFactory.eINSTANCE.createRoute();

				int index = 0;
				route.setNetTarget(parsedData.get(index++));
				route.setMask(parsedData.get(index++));
				route.setGateway(parsedData.get(index++));
				route.setInterface(parsedData.get(index++));
				route.setMetrics(Integer.parseInt(parsedData.get(index++)));

				route.setIp(route.getGateway());

				resolveHostName(gateway, route);
	 ...
	 * @param route
	 */
	private static void resolveHostName(final Device gateway, final Route route) {
		new Thread("Resolving host name.") {
			@Override
			public void run() {
				try {
					String hostName = InetAddress.getByName(route.getGateway()).getHostName();
					if (hostName != null) {
						route.setHost(hostName);
						gateway.getChildren().put(hostName, route);
					}
	 ...
	 * @return the name
	 */
	// public static String getName(Map map, eu.kalafatic.explorer.core.lib.EProject eProject) {
	// String result = "";
	//
